...
import { FirstTutorialModal } from '../Modals/FirstTutorialModal';
import { SecondTutorialModal } from '../Modals/SecondTutorialModal';
import { ThirdTutorialModal } from '../Modals/ThirdTutorialModal';
...
export const BattleScreen: React.FC<BattleScreenProps> = ({
    ...
    const [showInfoModal, setShowInfoModal] = useState(localStorage.getItem('isTutorialCompleted') !== 'true');
    const [tutorialPage, setTutorialPage] = useState(1);
    const logEndRef = useRef<HTMLDivElement>(null);

    const nextPage = () => {
        setTutorialPage((prev) => ++prev)
    }
    ...
    const TARGET_EXP = Math.random() * (1500 - 900) + 900; // You can make this dynamic based on enemies defeated if you wan
    useEffect(() => {
        if (battleState.phase === 'VICTORY') {
            let startTimestamp: number | null = null;
            const duration = 5000; // 5 seconds animation
            console.log(isLogOutButton);
            ...
            const step = (timestamp: number) => {
                ...
                setDisplayedExp(Math.floor(easeOut * TARGET_EXP));
            ...
            setDisplayedExp(0); // Reset if not victory
            setIsLogOutButton(true)
        }
    }, [battleState.phase]);
    ...
    if (battleState.phase === 'VICTORY') {
        return (
            <div className="h-screen w-full bg-yellow-400 flex flex-col items-center justify-center animate-fade-in relative overflow-hidden">
                ...
                {/* Victory Title */}
                <h1 className="text-6xl md:text-9xl font-display italic text-black transform -skew-x-12 mb-4 drop-shadow-lg">
                    VICTORY
                </h1>

                {/* EXP Counter */}
                <div className="bg-black text-white p-4 transform skew-x-12 border-4 border-white shadow-[8px_8px_0_rgba(0,0,0,0.5)] mb-8 animate-pop-in">
                    <div className="transform -skew-x-12 flex flex-col items-center">
                         <div className="text-sm md:text-xl font-bold text-yellow-400 tracking-widest">EXP GAINED</div>
                         <div className="text-5xl md:text-7xl font-display">{displayedExp}</div>
                    </div>
                </div>
                ...
                {isLogOutButton && (
                    <button 
                        onClick={onRestart} 
                        className="px-8 py-4 bg-black text-white font-bold text-2xl transform skew-x-12 hover:scale-110 transition-transform cursor-pointer z-50 flex items-center gap-2 border-2 border-white"
                    >
                        <LogOut size={24} className="transform -skew-x-12" />
                        <span className="transform -skew-x-12">RETURN TO MENU</span>
                    </button>
                )}
            </div>
        );
    ...
    return (
        <div className="h-screen w-full bg-[#222] relative overflow-hidden flex flex-col">
            ...
            <div className="flex-1 relative flex flex-col">
                ...
                {/* Heroes Area */}
                <div
                    className="flex items-end justify-center gap-1 md:gap-4 pb-20 md:pb-8 px-1 md:px-4 h-48 md:h-auto"
            ...
            {/* INFO MODAL */}
            <InfoModal isOpen={showInfoModal} onClose={() => setShowInfoModal(false)} title="BATTLE GUIDE">
                {tutorialPage === 1 && <FirstTutorialModal nextPage={nextPage} />}
                {tutorialPage === 2 && <SecondTutorialModal nextPage={nextPage} />}
                {tutorialPage === 3 && <ThirdTutorialModal closePage={exitTutorial} />}
            </InfoModal>
        </div>
    );
};
